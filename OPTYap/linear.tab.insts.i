...
/*-------------------------------------COMMOM IN WAM INSTRUCTIONS ------------------------*/
#define table_try_begin(){	 		 		 	      \
    tab_ent_ptr tab_ent;                                                      \
    sg_fr_ptr sg_fr;                                                          \
    check_trail(TR);                                                          \
...
STD_PROTO(static inline void table_try_with_complete, (sg_fr_ptr));
...
static inline void table_try_with_completed(sg_fr_ptr sg_fr){
...
** ------------------------------ */  
  ...
  PBOp(table_try_single, Otapl)
    ...
    if (SgFr_state(sg_fr) == ready) {
      ...
      /* subgoal completed */
      table_try_with_completed(sg_fr);
    }
  ENDPBOp();
